...
public abstract class TreeNode<T> : SimpleTreeNode<T> where T : TreeNode<T> {
	...
	///     If reordering is really needed, use <see cref="IExtendedList{T}.Swap" />,
...
///     If inheritance required, use <see cref="TreeNode{T}" /> instead.
/// </summary>
public sealed class ValuedTreeNode<T>(T value, ValuedTreeNode<T>? parent = null) : TreeNode<ValuedTreeNode<T>>(parent) {
